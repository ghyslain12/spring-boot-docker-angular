...
public class UserControllerAuthTest {
    ...
    void setUp() {
        ...
        if (count > 0) {
            throw new IllegalStateException("La table users n'est pas vide après deleteAll : " + count);
        }

        testUser = new User();
        testUser.setName("TestUser");
        testUser.setEmail("testuser_" + System.currentTimeMillis() + "@example.com"); // Email unique
    ...
    void testCreateUserWithAuthDuplicateEmail() throws Exception {
        String newUserJson = "{\"name\":\"NewUser\",\"email\":\"" + testUser.getEmail() + "\",\"password\":\"newpassword\"}";
